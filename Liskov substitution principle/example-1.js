// https://github.com/SamehEtman/SOLID-Principles-JS/tree/91a164ae7c5e35e0c85c92e463876133430511e3/Liskov%20Substitution%20principle
...
console.log(rc.toString()); //  2×3
let sq = new Square(5);
console.log(sq.toString()); //  5×5
...
let sq = new Square(5);
sq.width = 1;
console.log(sq.toString()); //1×5

// thats a little problem we can fix it by adding setters and getters for the square class like that
...
// Note : if you defined only the set function, the get function won't propagate from the parent so it's better to define setter too Now It's ouputs the desirable result
// 2×3
// 1×1

// Let's define a function that takes a rectangle and set its height to 10 then console.log its area note that it should be ok to pass a square to that function as it a subclass so it should work without a problem
...
// As you can see when we pass a square it doesn't behave as we meant the function to be, It looks like the function is specially made for rectangles only and we can't substitute it with its sub class Square
